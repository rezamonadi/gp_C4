...
filename = sprintf('%s/processed_qsos_%s.mat', ...
    ...
    training_set_name);
load(filename, 'sample_log_likelihoods_c4', 'map_N_c4', 'min_z_c4s', 'max_z_c4s');
...
for quasar_ind=1:num_quasars
    ...
    if(sum(this_systems)>0)
        ...
        if (this_c4s>0)
            ...
            NCIV_Cooksey_compare(jj) = matched_N_c4 - map_log_c4;
            ...
            %     map_log_c4
            % end
            Z_Cooksey_compare(jj) = matched_Z_c4 - map_z_c4;
        end
    end
end
fig=figure();
