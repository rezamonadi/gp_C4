filename = sprintf('%s/processed_qsos_%s', ...
    ...
    training_set_name);
load(filename, 'map_N_c4', 'map_z_c4', 'test_ind', 'p_c4', 'log_posteriors_c4',...
'log_posteriors_no_c4', 'map_sigma_c4')
% load(sprintf('FP-dist-%s/%s/catalog', processed_directory(release)), 'all_EW_c4');
cooksey_catalog = ...
fitsread('data/dr7/distfiles/dr7qso_CIV_noBAL.fit', 'binarytable');
all_zqso                = cooksey_catalog{4};
z_c13_test = all_zqso(test_ind);

z_separation = z_c13_test - map_z_c4;
...
% Z limitting 
x1 = p_c4(~ind_has_c4(test_ind) & map_z_c4<2.5);
x2 = p_c4(~ind_has_c4(test_ind) & map_z_c4>2.5 & map_z_c4<3.5);
x3 = p_c4(~ind_has_c4(test_ind) & map_z_c4>3.5);
fig=figure();
histogram(x1,50); 
legend('z<2.5')
...
% Sigma limitting 
x1 = p_c4(~ind_has_c4(test_ind) & map_sigma_c4'<25e5);
x2 = p_c4(~ind_has_c4(test_ind) & map_sigma_c4'>25e5 & map_sigma_c4'<35e5);
x3 = p_c4(~ind_has_c4(test_ind) & map_sigma_c4'>35e5);
fig=figure();
histogram(x1,50); 
legend('\sigma<25km/s')
set(get(gca, 'XLabel'), 'String', 'p-c4(False)');
...
%  N limitting
x1 = p_c4(~ind_has_c4(test_ind) & map_N_c4<14);
x2 = p_c4(~ind_has_c4(test_ind) & map_N_c4>14 & map_N_c4<15);
x3 = p_c4(~ind_has_c4(test_ind) & map_N_c4>15);
fig=figure();
histogram(x1,50); 
legend('N<13')
set(get(gca, 'XLabel'), 'String', 'p-c4(False)');
